...
# Reuse installed deps (root, webapp, and ui) so package-level bins are present
...
# Build UI first (generates CSS), then the webapp
...
COPY --from=builder /app/apps/webapp/next.config.ts ./next.config.ts
